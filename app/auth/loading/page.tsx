      ...
      <div className="max-w-md w-full">
        <div className="bg-gray-900/50 backdrop-blur-xl border border-white/10 rounded-2xl p-8 text-center">
          ...
          <div className="w-full bg-gray-700 rounded-full h-1.5 mb-4">
            <div
              className="bg-gradient-to-r from-purple-600 to-purple-400 h-1.5 rounded-full animate-pulse"
              style={{ width: "30%" }}
            ></div>
          </div>
...
// Main auth loading component that uses useSearchParams
    ...
    if (error) {
      ...
      const errorTimeout = setTimeout(() => {
        if (status === "loading") {
          setStatus("error");
          setMessage(
            "Authentication is taking longer than expected. This might be due to a slow connection."
          );
          setTimeout(() => {
            router.push("/signin");
          }, 4000);
