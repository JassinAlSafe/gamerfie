-- Drop all versions of create_challenge function
DO $$ 
BEGIN
  -- Drop all versions of the function (with different parameter lists)
  DROP FUNCTION IF EXISTS create_challenge(JSONB, JSONB[], JSONB[], JSONB[], TEXT);
  DROP FUNCTION IF EXISTS create_challenge(challenge_input, goal_input[], reward_input[], rule_input[], TEXT);
  DROP FUNCTION IF EXISTS create_challenge(JSONB, JSONB[], JSONB[], JSONB[]);
  DROP FUNCTION IF EXISTS create_challenge(challenge_input, goal_input[], reward_input[], rule_input[]);
END $$;

-- Drop types if they exist
DROP TYPE IF EXISTS challenge_input CASCADE;
DROP TYPE IF EXISTS goal_input CASCADE;
DROP TYPE IF EXISTS reward_input CASCADE;
DROP TYPE IF EXISTS rule_input CASCADE;

-- Create the stored procedure with JSONB parameters
